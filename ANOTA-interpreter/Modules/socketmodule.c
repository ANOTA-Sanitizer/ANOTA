...
#define ANOTA_HOST_KIND_DOMAIN "DOMAIN"
#define ANOTA_HOST_KIND_IP "IP"

static PyObject *
anota_socket_extract_host(PyObject *address)
{
    if (address == NULL) {
        return NULL;
    }
    if (PyTuple_Check(address) && PyTuple_GET_SIZE(address) > 0) {
        PyObject *host = PyTuple_GET_ITEM(address, 0);
...
static int
anota_socket_is_ip_literal(PyObject *host)
{
    Py_ssize_t len;
    const char *text = PyUnicode_AsUTF8AndSize(host, &len);
    if (text == NULL) {
        return -1;
    }
    if (len == 0) {
...
static int
anota_socket_check_address(const char *syscall_name,
                           const char *operation,
                           PyObject *address)
{
    PyObject *host = anota_socket_extract_host(address);
    if (host == NULL) {
        if (PyErr_Occurred()) {
            return -1;
        }
    ...
    int ip_literal = anota_socket_is_ip_literal(host);
    if (ip_literal < 0) {
        Py_DECREF(host);
        return -1;
    }
    const char *kind = ip_literal ? ANOTA_HOST_KIND_IP : ANOTA_HOST_KIND_DOMAIN;
    int rc = _PyAnotaSyscall_Check(syscall_name, host, operation, kind);
    Py_DECREF(host);
    if (rc < 0) {
        return -1;
    }
...
sock_bind(PySocketSockObject *s, PyObject *addro)
{
    ...
    if (anota_socket_check_address("bind", "BIND", addro) < 0) {
        return NULL;
    }
...
sock_connect(PySocketSockObject *s, PyObject *addro)
{
    ...
    if (anota_socket_check_address("connect", "CONNECT", addro) < 0) {
        return NULL;
    }
...
sock_initobj(PyObject *self, PyObject *args, PyObject *kwds)
{
    ...
    if (fdobj != NULL && fdobj != Py_None) {
        ...
        PyObject *proto_value = PyLong_FromLong(proto);
        if (proto_value == NULL) {
            return -1;
        }
        int proto_check = _PyAnotaSyscall_Check(
            "socket", proto_value, "SOCKET", "PROTOCOL");
        Py_DECREF(proto_value);
        if (proto_check < 0) {
            return -1;
        }
#ifdef MS_WINDOWS
