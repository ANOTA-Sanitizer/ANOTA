...
/*[clinic end generated code: output=aefafc4ce2b46dc0 input=7295902222e6b311]*/
{
    ...
    int need_read_check = 0, need_write_check = 0;
    ...
    policy_path = is_number ? NULL : path_or_fd;
    need_read_check = reading || updating;
    need_write_check = writing || appending || creating || updating;

    if (need_read_check) {
        if (_PyAnotaSyscall_Check("OPEN", policy_path, "READ", "PATH") < 0) {
            goto error;
        }
    }
    if (need_write_check) {
        if (_PyAnotaSyscall_Check("OPEN", policy_path, "WRITE", "PATH") < 0) {
            goto error;
        }
    }
...
{
    if (_PyAnotaSyscall_Check("OPEN_CODE", path, "READ", "PATH") < 0) {
        return NULL;
    }
