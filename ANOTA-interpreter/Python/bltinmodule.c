...
PyDoc_STRVAR(anota_syscall_signal_start_doc,
"ANOTA_SYSCALL_SIGNAL_START(pid=None)\n\
\n\
Request the external syscall monitor to begin tracing. If *pid* is provided,\n\
only syscalls from that process will be observed; otherwise, all processes are traced.");

PyDoc_STRVAR(anota_syscall_signal_stop_doc,
"ANOTA_SYSCALL_SIGNAL_STOP()\n\
\n\
Request the external syscall monitor to stop tracing.");
...
static PyMethodDef builtin_methods[] = {
    ...
    {"ANOTA_SYSCALL_SIGNAL_STOP", (PyCFunction)_PyAnotaSyscall_SignalStop,
     METH_NOARGS, anota_syscall_signal_stop_doc},
...
_PyBuiltin_Init(PyInterpreterState *interp)
{
    ...
    /* Expose ANOTA_WATCH singleton in builtins so that user code can write
       ANOTA_WATCH.ALLOW(obj, "R"), etc. */
    {
        PyObject *aw = _PyAnotaWatch_GetSingleton();
        if (aw == NULL) {
            Py_DECREF(mod);
            return NULL;
        }
        if (PyDict_SetItemString(dict, "ANOTA_WATCH", aw) < 0) {
            Py_DECREF(aw);
            Py_DECREF(mod);
            return NULL;
        }
    ...
    /* Expose ANOTA_EXECUTION singleton in builtins so that user code can write
       ANOTA_EXECUTION.BLOCK(cond, msg=None). */
    {
        PyObject *ae = _PyAnotaExecution_GetSingleton();
        if (ae == NULL) {
            Py_DECREF(mod);
            return NULL;
        }
        if (PyDict_SetItemString(dict, "ANOTA_EXECUTION", ae) < 0) {
            Py_DECREF(ae);
            Py_DECREF(mod);
            return NULL;
        }
    ...
    /* Expose ANOTA_SYSCALL singleton to allow syscall policies to be tweaked. */
    {
        PyObject *as = _PyAnotaSyscall_GetSingleton();
        if (as == NULL) {
            Py_DECREF(mod);
            return NULL;
        }
        if (PyDict_SetItemString(dict, "ANOTA_SYSCALL", as) < 0) {
            Py_DECREF(as);
            Py_DECREF(mod);
            return NULL;
        }
