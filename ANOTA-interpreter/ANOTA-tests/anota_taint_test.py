...
def test_25_unhashable_objects_are_tainted_by_identity():
    """
    Taint tracking should be object-based, not limited to hashable objects.
    """
    taint = get_anota_taint()
...
def test_35_tainted_callable_taints_return_value():
    """
    The paper describes tainted callables as tainting the objects they return.
    """
    taint = get_anota_taint()
    ...
    taint(secret_factory, Sink=[print])

    produced = secret_factory()
    expect_runtime_error(print, produced)
...
TESTS = [
    ...
    ("25_unhashable_objects_are_tainted_by_identity", test_25_unhashable_objects_are_tainted_by_identity),
    ("30_sanitizer_hash_clears_taint_for_result", test_30_sanitizer_hash_clears_taint_for_result),
    ("35_tainted_callable_taints_return_value", test_35_tainted_callable_taints_return_value),
