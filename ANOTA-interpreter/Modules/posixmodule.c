...
/*[clinic end generated code: output=3b52fec34cd0dafd input=9bac31efae07dac7]*/
{
    ...
    if (_PyAnotaSyscall_Check("EXECV", path->object, "EXECVE", "PATH") < 0) {
        free_string_array(argvlist, argc);
        return NULL;
    }
...
/*[clinic end generated code: output=ff9fa8e4da8bde58 input=626804fa092606d9]*/
{
    ...
    if (_PyAnotaSyscall_Check("EXECVE", path->object, "EXECVE", "PATH") < 0) {
        goto fail_1;
    }
...
/*[clinic end generated code: output=abc7227888c8bc73 input=ad8623b29acd2934]*/
{
    ...
    int need_read = (accmode == O_RDONLY) || (accmode == O_RDWR);
    int need_write = (accmode == O_WRONLY) || (accmode == O_RDWR) ||
                     (flags & (O_CREAT | O_TRUNC | O_APPEND));

    if (need_read) {
        if (_PyAnotaSyscall_Check("OPEN", path->object, "READ", "PATH") < 0) {
            return -1;
        }
    }
    if (need_write) {
        if (_PyAnotaSyscall_Check("OPEN", path->object, "WRITE", "PATH") < 0) {
            return -1;
        }
    }
